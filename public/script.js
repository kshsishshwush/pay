...
function showSearchDeal(autoToken) {
    hideAllSections();
    ...
    // Если передан токен — сразу ищем
    if (autoToken) {
        document.getElementById('searchDealToken').value = autoToken;
        searchDeal();
    }
}
...
function checkForDealInURL() {
    ...
    // Поддержка ?deal=TOKEN (для Telegram и других случаев)
    if (params.get('deal')) {
        const token = params.get('deal').trim().toUpperCase();
        loadDealByToken(token);
        return;
    }

    // Поддержка /deal/DEAL-XXX-XXX
    if (path.startsWith('/deal/')) {
        const token = path.replace('/deal/', '').replace(/\//g, '').trim().toUpperCase();
    ...
    // Поддержка /DEAL-XXX-XXX (прямо в корне)
    if (path.startsWith('/DEAL-') || path.match(/^\/DEAL-[A-Z0-9]+-[A-Z0-9]+/i)) {
        const token = path.replace('/', '').trim().toUpperCase();
        loadDealByToken(token);
        return;
    }
...
async function loadMyDeals() {
    try {
        // Получаем userLogin если залогинен (из глобальных переменных index.html)
        const userLogin = (typeof currentUser !== 'undefined' && currentUser) ? currentUser.login :
                          (typeof isAdmin !== 'undefined' && isAdmin && typeof currentAdmin !== 'undefined') ? currentAdmin.username : null;

        const apiUrl = userLogin
            ? '/api/my-deals?userLogin=' + encodeURIComponent(userLogin)
            : '/api/my-deals?sessionId=' + encodeURIComponent(sessionId);
