...
// Telegram/соцсети читают OG теги при создании превью — до запуска JS.
// Поэтому для /DEAL-* мы читаем сделку из БД и вставляем правильные OG теги в HTML.
app.get('/DEAL-:dealCode', async (req, res) => {
  ...
  try {
    ...
    if (deal) {
      const methodText = deal.method === 'phone' ? 'по номеру телефона'
                       : deal.method === 'card'  ? 'по номеру карты'
                       : 'по лицевому счёту';
      const currency = deal.currency_symbol || '₽';
      const title = `Пополнение ${deal.bank} ${methodText} на ${deal.amount} ${currency}`;
      const desc  = `Получатель: ${deal.recipient} · Payments Банк`;

      // Выбираем превью по банку
      const previewMap = {
      ...
      console.log(`[OG] ✅ Injected OG tags for ${token}: ${title}`);
    } else {
      console.log(`[OG] Deal not found: ${token}, serving default`);
    }
  } catch (e) {
    console.error('[OG] Error fetching deal:', e.message);
  }
