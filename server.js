...
app.patch('/api/deal/:token', async (req, res) => {
  try {
    ...
    // Добавляем поля только если они есть в таблице
    try { updateObj.payment_data = paymentData; } catch(e) {}
    ...
    if (error) {
      ...
      // Пробуем обновить только статус
      ...
      if (error2) {
        console.error('Supabase PATCH status-only error:', error2);
        return res.status(500).json({ error: 'Ошибка базы данных' });
      }
      return res.json({ success: true, deal: data2[0] });
    }
